    ...
    return (
        <div style={{ marginLeft: '420px', paddingTop: '20px' }}>
            <Stack direction={"row"} sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap' }}>
                {filmItems.map((item) => (
                    <FilmCard name={item.nameRu || item.nameOriginal} poster={item.posterUrl} year={item.year} id={item.kinopoiskId} favourites={favourites} setFavourites={setFavourites} key={item.kinopoiskId} />
                ))}
            </Stack>
        </div>
    )
